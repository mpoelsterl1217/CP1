...
def match(pattern: List[str], source: List[str]) -> List[str]:
    ...
    # while pind is still a valid index OR sind is still a valid index
    #    (valid index means that the index is != to the length of the list)
        ...
        # 2) check to see if the current thing in the pattern is a %
            ...
            if pind == (len(pattern) - 1):
                ...
                while pattern[pind] != source[sind]:
                    ...
                    # abort in case we've run out of source
                    # but there's still more pattern left
                    if sind >= len(source): return None

...
def title_by_year(matches: List[str]) -> List[str]:
    ...
    """
    result: List[str] = []
    for (t, d, y, a) in movie_db:
        if y == int(matches[0]):
            result.append(t)
    return result

assert sorted(title_by_year(["1974"])) == sorted(['amarcord', 'chinatown']), "failed title_by_year test 1"
assert sorted(title_by_year(["1992"])) == sorted(['the crying game', 'flirting', 'malcolm x']), "failed title_by_year test 2"
...
def title_by_year_range(matches: List[str]) -> List[str]:
    ...
    Returns:
        ...
        pass in ["1991", "1994"] you will get movies made in 1991, 1992, 1993 & 1994)
    """
    result: List[str] = []
...
def title_before_year(matches: List[str]) -> List[str]:
    ...
    Returns:
        a list of movie titles made after the passed in year, exclusive (meaning
        if you pass in 1992 you won't get any movies made that year, only after)
    """
    result: List[str] = []
...
assert sorted(actors_by_title(["jaws"])) == sorted(['roy scheider', 'robert shaw', 'richard dreyfuss', 'lorraine gary', 'murray hamilton']), "failed actors_by_title test"
...
assert sorted(year_by_title(["jaws"])) == sorted([1975]), "failed year_by_title test"
...
assert sorted(title_by_actor(["orson welles"])) == sorted(['citizen kane', 'othello']), "failed title_by_actor test"
...
pa_list: List[Tuple[List[str], Callable[[List[str]], List[Any]]]] = [
    ...
    # note there are two valid patterns here
    # two different ways to ask for the director of a movie
...
def search_pa_list(src: List[str]) -> List[str]:
    ...
    result: List[str] = []
    find = None
    for (question, search) in pa_list:
        found = match(question, src)
        if found != None:
            find = search(found)
            result = find
    if find == None:
...
assert sorted(search_pa_list(["who", "directed", "jaws"])) == sorted(['steven spielberg']), "failed search_pa_list test 2"
assert sorted(search_pa_list(["what", "movies", "were", "made", "in", "2020"])) == sorted(['No answers']), "failed search_pa_list test 3"
